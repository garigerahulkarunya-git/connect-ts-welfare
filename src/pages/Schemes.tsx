  ...
  return (
    <Layout>
      ...
      <section className="bg-background py-10 md:py-14">
        <div className="container">
          ...
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-foreground leading-tight mt-4">
            {"Explore Government Welfare Schemes".split(" Government ").map((part, i, arr) => i === 0 && arr.length > 1 ? <>{part} Government <br/> {arr[1]}</> : part)}
            ...
            <div className="flex gap-2 flex-wrap">
              <div className="relative">
                <select
                  ...
                  className="h-11 rounded-lg border border-border bg-card text-foreground px-3 pr-8 text-sm appearance-none focus:ring-2 focus:ring-primary focus:outline-none w-full sm:w-auto"
                >
              ...
              <select
                ...
                className="h-11 rounded-lg border border-border bg-card text-foreground px-3 pr-2 text-sm appearance-none focus:ring-2 focus:ring-primary focus:outline-none w-full sm:w-auto"
              >
                      ...
                      {scheme.status === "OPEN" ? (
                        ...
                        <Link to="/apply" className="flex-1">
                          <Button variant="outline" className="w-full">
                            Apply {scheme.deadline ? new Date(scheme.deadline).toLocaleDateString("en-IN", { month: "short", day: "numeric" }) : "Coming Soon"}
                          </Button>
                        </Link>
                      )}
      ...
      <section className="py-16 md:py-20">
        <div className="container">
          <div className="gov-gradient rounded-3xl p-8 md:p-12 grid md:grid-cols-2 gap-6 md:gap-8 items-center">
            ...
            <div className="bg-card rounded-2xl p-6 shadow-lg">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Applicant's Age</label>
                    <Input placeholder="e.g. 24" className="mt-1 h-10" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Primary Need / Purpose</label>
                    <select className="mt-1 w-full rounded-lg border border-border bg-background text-foreground px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:outline-none">
                ...
                <Button className="w-full h-11 font-semibold text-base">
                  Get AI Recommendations <Zap className="ml-2 h-4 w-4" />
                </Button>
                <p className="text-[10px] text-center text-muted-foreground uppercase tracking-widest">Data processed securely & confidentially</p>
              </div>
            </div>
          </div>
        </div>
