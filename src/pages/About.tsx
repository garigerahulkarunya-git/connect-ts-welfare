    ...
    <Layout>
      ...
      <section className="section-padding">
        <div className="container text-center">
          ...
          <div className="grid gap-6 grid-cols-1 sm:grid-cols-3 max-w-3xl mx-auto">
            {officials.map((person, i) => (
              <div key={person.name} className="flex flex-col items-center">
                <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full bg-muted flex items-center justify-center text-3xl font-bold text-primary mb-4 border-4 border-background shadow-md overflow-hidden relative">
                  {person.photo ? (
                    <img
                      src={person.photo}
                      alt={person.name}
                    ...
                    <span>{person.name.split(" ").pop()?.charAt(0)}</span>
                  )}
                </div>
                <h3 className="font-bold text-sm">{person.name}</h3>
                <p className="text-xs text-muted-foreground mt-1">{person.designation}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="container text-center max-w-3xl">
          ...
          <div className="flex flex-col items-center space-y-4">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {[
                "District Minority Welfare Officers",
                "Admin & Accounting Wing",
          ...
          <div className="grid md:grid-cols-2 gap-10 items-center">
            <div>
              <h2 className="text-3xl md:text-4xl font-extrabold leading-tight mb-4">
                {"Quantifiable Social Progress".split(" ").map((w,i)=><span key={i}>{w} {i===0&&<br/>}</span>)}
              </h2>
        ...
        <div className="container max-w-4xl">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                { label: "Annual Report 2025-26", icon: "📄" },
                { label: "Policy Framework", icon: "📋" },
                { label: "Budget Allocation", icon: "💰" },
                { label: "Audit Reports", icon: "📊" },
              ].map((doc) => (
                <Card key={doc.label} className="cursor-pointer hover:shadow-md transition-shadow shadow-none">
